...
def update2text(update: Update, locale):  # locale="ru_RU" or "en_US"
    message = update.message
    bot = update.message.bot

    # если в сообщении есть текст, то берём его для начала
    text = message.text

    # если есть голос, то попробуем его распознать
    if message.voice:
        file_info = bot.get_file(message.voice.file_id)
        file = requests.get(
            'https://api.telegram.org/file/bot{0}/{1}'.format(BOT_API_TOKEN,
                                                              file_info.file_path))  # вроде так из телеги файлы выкачиваются
